...
** Me
- Rune Kaagaard
- Lead Developer at Prescriba
- Building AI-powered developer tools
- MCP Alchemy: Database MCP server for Claude

** This Talk
- Real workflows that work in production
- Documentation strategies for large codebases
- Automation and tooling patterns
- What Anthropic learned building Claude Code with Claude Code

* Getting Started
** Talk
- npm install -g @anthropic-ai/claude-code
- Three modes: Normal, Plan, Auto-accept
...
* Philosophy
** Talk
- Claude = fast mid-level dev with perfect memory
- You shift from engineer to agent manager
- Needs direction, reviews, guardrails
- Not a senior architect (yet)
...
for app in src/apps/*; do
  ...
  repomix --include "$app/**" --style markdown \
    --output "packed-$appname.md"
  claude -p -c "Read @packed-$appname.md and \
    @llm/how_to_document_an_app.md then \
...
- Tell Claude NOT to code yet
- Use @ for file references
- Build understanding incrementally

*** Claude
#+begin_src markdown
Read and understand the auth system:
@src/services/auth.ts
@src/middleware/auth.ts

Don't make changes yet. Just explore.
#+end_src
...
*** Talk
Root and Leaves:
- Root: API interfaces, types, test scaffolding
- Leaves: UI, boilerplate, implementation details
- Choose based on problem

*** Claude
#+begin_src markdown
Write tests for OAuth flow first.
Then implement to make tests pass.
#+end_src

*** Feel
- I'm in control
- Not surprised by output
...
clg() {
  claude -p "Generate conventional commit for staged changes"
}
#+end_src
...
- Everything counts (15K baseline)
- Performance degrades after 80%
...
- Stay under 80% for quality
- Multiple short sessions > one long session
- Claude 4.5+ discovers state from filesystem

* Built-in Tools
** Talk
- File operations: Read, Write, Edit, Grep
- Execution: Bash, npm, pip, cargo
- Testing: Vitest, Jest, Pytest
- Version control: Full git operations
- Web access: WebFetch, Curl

** Permissions
*** Talk
- Auto-allowed, Prompt, or Denied
...
- Database access, browser automation, APIs
- Configure in .mcp.json
- Use when CLI tools insufficient
